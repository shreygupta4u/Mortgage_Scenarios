"""pages/scenario_editor.py
Shared dialogs + compute helpers used by tab_scenarios, tab_prepayment, tab_comparison.
"""
...
def apply_prepay_settings(settings, b, base_extras=None):
    """Convert prepayment scenario settings dict → extra_payments list."""
    result = list(base_extras or [])
    al  = float(settings.get("annual_lump", 0))
    lm  = int(settings.get("lump_month", 1))
    sy  = int(settings.get("lump_start_year", 1))
    ny  = int(settings.get("lump_num_years", 0))
    if al > 0:
    ...
    if inc_t == "Fixed" and inc_v > 0:
        for p in range(1, total_p + 1):
            result.append({"period": p, "amount": inc_v})
    elif inc_t in ("Pct", "% increase") and inc_v > 0:
        base_pmt = calc_pmt(b["principal"], b["annual_rate"], b["n_py"], b["amort_years"], b["accel"])
        for p in range(1, total_p + 1):
            result.append({"period": p, "amount": base_pmt * inc_v / 100})
    otp = int(settings.get("onetime_period", 0))
    ota = float(settings.get("onetime_amount", 0))
    if otp > 0 and ota > 0:
        result.append({"period": otp, "amount": ota})
    return result


def compute_scenario(sc, b, prepay_sc=None):
    """
    Build amortization for a rate-change scenario + optional prepayment scenario.
    Returns (df_sc, s_sc, all_rcs, sc_extra, last_rate, sc_term_end_p).
    """
    ...
    for rn in sc.get("renewals", []):
        ...
        if amt > 0:
            sc_extra.append({"period": int(rn["period"]), "amount": amt})
    if prepay_sc:
    ...
    sc_term_end_p = (
        int(sc["renewals"][-1]["period"]) + int(float(sc["renewals"][-1]["term_years"]) * b["n_py"])
        if sc.get("renewals") else b["orig_term_end_p"]
    )
    return df_sc, s_sc, all_rcs, sc_extra, last_rate, sc_term_end_p


def compute_adj_scenario(sc, b, prepay_sc, df_base, s_base):
    """
    Full result tuple for a scenario + prepayment.
    Returns (s_adj, adj_rem, adj_end, delta_yrs, colour, calc_monthly_sc).
    """
...
def edit_scenario_dialog():
    ...
    # ── Name + description (single row) ──────────────────────────
    d1, d2 = st.columns([2, 3])
    sc["name"] = d1.text_input("Scenario Name", sc["name"], key=f"dlg_name_{sc_id}",
                                help="Name used to identify and save this scenario")
    ...
    pmt_c1, pmt_c2 = st.columns([2, 3])
    user_pmt = pmt_c1.number_input(
        "Amount ($)", min_value=min_pmt, max_value=float(today_bal),
        ...
        help=f"Required to maintain current amort at {last_rate:.2f}%: ${calc_monthly_sc:,.2f}\n"
             "Increase to pay off faster; set to required for baseline comparison."
    )
    sc["user_pmt"] = user_pmt
    pmt_c2.info(f"Required payment (baseline): **${calc_monthly_sc:,.2f}**  \n"
                f"Difference: **${user_pmt - calc_monthly_sc:+,.2f}/period**")

    st.divider()
    st.markdown("##### 🔄 Rate Renewals")

    # Base schedule for early-penalty calcs
    ...
    for ri, rn in enumerate(sc.get("renewals", [])):
        ...
        with st.container(border=True):
            ...
            rn["mtype"]    = c3.selectbox("Type", ["Fixed", "Variable"],
                                           ...
                                           help="Fixed locks rate; Variable floats")
            rn["new_rate"] = float(c4.number_input("Rate (%)", 0.5, 20.0,
                                   ...
                                   help="Annual interest rate for this term"))
            ...
            if c6.button("🗑️", key=f"dlg_delren_{sc_id}_{rid}", help="Remove renewal"):
                ren_del.append(ri)
            ...
            if is_early:
                ...
                rn["orig_posted"] = float(bp1.number_input(
                    ...
                    key=f"dlg_op_{sc_id}_{rid}", help="Bank's posted rate at origination"))
                rn["curr_posted"] = float(bp2.number_input(
                    ...
                    key=f"dlg_cp_{sc_id}_{rid}", help="Current posted rate for remaining term"))
                ...
                # FIX #2: read from widget key directly — guarantees latest typed value is saved
                wkey = f"dlg_pen_txt_{sc_id}_{rid}"
                if pkey not in st.session_state:
                    st.session_state[pkey] = str(int(adv["calc_penalty"]))
                pen_str = st.text_input(
                    f"Penalty to apply ($) — max ${adv['calc_penalty']:,.0f}",
                    value=st.session_state[pkey], key=wkey,
                    help="Edit to the actual penalty your bank quoted")
                # Always read back from widget key (reliable on Save click)
                current_pen = st.session_state.get(wkey, pen_str)
                st.session_state[pkey] = current_pen
                try:
                    rn["actual_penalty"] = float(current_pen.replace(",", "").replace("$", ""))
                except Exception:
                    rn["actual_penalty"] = adv["calc_penalty"]

                rn["misc_fees"] = float(st.number_input(
                    "Misc fees ($)", 0, 50_000, int(rn.get("misc_fees", 500)), 50,
                    key=f"dlg_mf_{sc_id}_{rid}", help="Admin, appraisal, legal fees"))
                ...
                rn["misc_fees"]      = float(st.number_input(
                    ...
                    key=f"dlg_mf2_{sc_id}_{rid}", help="Fees at normal renewal"))
                rn["actual_penalty"] = 0
    ...
    st.divider()
    # ── Save / Close (req #3: use scenario name, no second name field) ──
    ba1, ba2 = st.columns(2)
    if ba1.button("💾 Save to DB", key=f"dlg_save_{sc_id}",
                   use_container_width=True, type="primary",
                   help="Saves under the Scenario Name above"):
        if not sc["name"].strip():
            st.error("❌ Scenario Name cannot be empty.")
        else:
            pp_settings = {"annual_lump": 0, "lump_month": 1, "lump_start_year": 1,
                           "lump_num_years": 0, "pay_increase_type": "None",
                           "pay_increase_val": 0, "onetime_period": 0, "onetime_amount": 0}
            db_id = db_save_scenario(
                conn, sc.get("db_id"), sc["name"].strip(), sc.get("desc", ""),
                sc["renewals"], pp_settings, sc.get("user_pmt", 0),
            )
            if db_id:
                sc["db_id"] = db_id
                st.session_state["_editing_sc_id"] = None
    ...
    if ba2.button("✕ Close without saving", key=f"dlg_close_{sc_id}",
                   use_container_width=True):
        st.session_state["_editing_sc_id"] = None
        st.rerun()


# ── Prepayment scenario dialog ────────────────────────────────────
...
def edit_prepay_dialog():
    ...
    sc["name"] = d1.text_input("Scenario Name", sc["name"], key=f"pp_dlg_name_{sc_id}",
                                help="Name this prepayment strategy")
    sc["desc"] = d2.text_area("Description", sc.get("desc", ""), height=68,
    ...
    s["annual_lump"] = float(al1.number_input("Amount ($)", 0, 500_000,
                             int(s.get("annual_lump", 0)), 500, key=f"pp_dlg_al_{sc_id}",
                             help="Annual principal prepayment amount"))
    if s["annual_lump"] > 0:
    ...
    inc_t = st.radio("Type", inc_opts, index=inc_opts.index(inc_cur), horizontal=True,
                      key=f"pp_dlg_it_{sc_id}", help="How to increase regular payments")
    s["pay_increase_type"] = "Fixed" if inc_t == "Fixed $" else inc_t
    if inc_t == "Fixed $":
        s["pay_increase_val"] = float(st.number_input("Extra per payment ($)", 0, 10_000,
                                      int(s.get("pay_increase_val", 0)), 50,
                                      key=f"pp_dlg_if_{sc_id}", help="Fixed extra $ added each payment"))
    elif inc_t == "% increase":
        s["pay_increase_val"] = float(st.slider("% increase", 1, 100,
                                      int(s.get("pay_increase_val", 10)),
                                      key=f"pp_dlg_ip_{sc_id}", help="% of required payment to add"))
    else:
    ...
    s["onetime_period"] = int(ot1.number_input("Period #", 0, int(b["amort_years"] * b["n_py"]),
                              ...
                              help="Payment period for the one-time lump (0 = disabled)"))
    s["onetime_amount"] = float(ot2.number_input("Amount ($)", 0, 2_000_000,
                                int(s.get("onetime_amount", 0)), 1_000,
                                key=f"pp_dlg_oa_{sc_id}", help="One-time lump sum amount"))
    ...
    st.divider()
    ba1, ba2 = st.columns(2)
    if ba1.button("💾 Save to DB", key=f"pp_dlg_save_{sc_id}",
                   use_container_width=True, type="primary"):
        if not sc["name"].strip():
            st.error("❌ Name cannot be empty.")
        else:
            db_id = db_save_prepay_scenario(conn, sc.get("db_id"),
                                             sc["name"].strip(), sc.get("desc", ""), s)
            if db_id:
                sc["db_id"] = db_id
                st.session_state["_editing_pp_sc_id"] = None
                st.rerun()
