"""mortgage_db.py — DB helpers supporting BOTH MS SQL Server (pyodbc) AND SQLite."""
import os, uuid as _uuid_mod, sqlite3


# ── Thin connection wrapper ───────────────────────────────────────
class _Conn:
    """Wraps pyodbc or sqlite3 connection. Carries db_type so helpers can adapt."""
    def __init__(self, raw, db_type, label=""):
        self._raw    = raw
        self.db_type = db_type   # "mssql" | "sqlite"
...
def _top1(conn, col_sql, table_sql, order_sql=""):
    """SELECT TOP 1 ... FROM ... ORDER BY ..."""
    if conn.db_type == "sqlite":
        return f"SELECT {col_sql} FROM {table_sql} {'ORDER BY '+order_sql if order_sql else ''} LIMIT 1"
    return f"SELECT TOP 1 {col_sql} FROM {table_sql} {'ORDER BY '+order_sql if order_sql else ''}"

def _getdate(conn):
    return "datetime('now')" if conn.db_type == "sqlite" else "GETDATE()"

def _autoincrement(conn):
    return "INTEGER PRIMARY KEY AUTOINCREMENT" if conn.db_type == "sqlite" else "INT IDENTITY PRIMARY KEY"

def _text(conn):
    return "TEXT" if conn.db_type == "sqlite" else "NVARCHAR"
...
def get_sqlite_connection(path=None):
    ...
    try:
        if not path:
            path = os.path.join(os.getcwd(), "mortgage_local.db")
        raw  = sqlite3.connect(path, check_same_thread=False)
        raw.row_factory = sqlite3.Row   # dict-like rows
...
def _create_tables(conn):
    ai  = _autoincrement(conn)
    gd  = _getdate(conn)
    c   = conn.cursor()
    stmts = []
    if conn.db_type == "sqlite":
        stmts = [
            f"CREATE TABLE IF NOT EXISTS mortgage_setup(id {ai}, purchase_price REAL NOT NULL, "
            "down_pct REAL NOT NULL DEFAULT 20, mortgage_type TEXT NOT NULL DEFAULT 'Fixed', "
            "pay_frequency TEXT NOT NULL DEFAULT 'Monthly', annual_rate REAL NOT NULL, "
            "amort_years INTEGER NOT NULL DEFAULT 30, term_years REAL NOT NULL DEFAULT 3, "
            f"start_date TEXT NOT NULL, include_cmhc INTEGER NOT NULL DEFAULT 1, saved_at TEXT DEFAULT ({gd}))",

            f"CREATE TABLE IF NOT EXISTS mortgage_past_renewals(id {ai}, setup_id INTEGER, seq_num INTEGER, "
            "start_date TEXT, annual_rate REAL, mortgage_type TEXT, term_years REAL)",

            f"CREATE TABLE IF NOT EXISTS mortgage_past_prepayments(id {ai}, setup_id INTEGER, seq_num INTEGER, "
            "payment_date TEXT, amount REAL)",

            f"CREATE TABLE IF NOT EXISTS mortgage_scenarios(id {ai}, name TEXT, description TEXT DEFAULT '', "
            "annual_lump REAL DEFAULT 0, lump_month INTEGER DEFAULT 1, lump_start_year INTEGER DEFAULT 1, "
            "lump_num_years INTEGER DEFAULT 0, pay_increase_type TEXT DEFAULT 'None', pay_increase_val REAL DEFAULT 0, "
            "onetime_period INTEGER DEFAULT 0, onetime_amount REAL DEFAULT 0, user_pmt REAL DEFAULT 0, "
            f"linked_pp_db_id INTEGER DEFAULT 0, created_at TEXT DEFAULT ({gd}), updated_at TEXT DEFAULT ({gd}))",

            f"CREATE TABLE IF NOT EXISTS mortgage_scenario_renewals(id {ai}, scenario_id INTEGER, "
            "seq_num INTEGER, mode TEXT DEFAULT 'By Date', effective_date TEXT, effective_period INTEGER DEFAULT 1, "
            "new_rate REAL, mortgage_type TEXT DEFAULT 'Fixed', term_years REAL DEFAULT 3, "
            "actual_penalty REAL DEFAULT 0, misc_fees REAL DEFAULT 250, "
            "orig_posted_rate REAL DEFAULT 0, curr_posted_rate REAL DEFAULT 0, onetime_amount REAL DEFAULT 0)",

            f"CREATE TABLE IF NOT EXISTS mortgage_prepay_scenarios(id {ai}, name TEXT NOT NULL, "
            "description TEXT DEFAULT '', annual_lump REAL DEFAULT 0, lump_month INTEGER DEFAULT 1, "
            "lump_start_year INTEGER DEFAULT 1, lump_num_years INTEGER DEFAULT 0, "
            "pay_increase_type TEXT DEFAULT 'None', pay_increase_val REAL DEFAULT 0, "
            f"onetime_period INTEGER DEFAULT 0, onetime_amount REAL DEFAULT 0, created_at TEXT DEFAULT ({gd}), updated_at TEXT DEFAULT ({gd}))",
        ]
    else:
        stmts = [
            ...
            "user_pmt DECIMAL(10,2) DEFAULT 0, linked_pp_db_id INT DEFAULT 0, "
...
def _run_migrations(conn):
    ...
    if conn.db_type == "sqlite":
        migs = [
            ("onetime_amount_ren",  "ALTER TABLE mortgage_scenario_renewals ADD COLUMN onetime_amount REAL DEFAULT 0"),
            ("user_pmt_sc",        "ALTER TABLE mortgage_scenarios ADD COLUMN user_pmt REAL DEFAULT 0"),
            ("linked_pp_sc",       "ALTER TABLE mortgage_scenarios ADD COLUMN linked_pp_db_id INTEGER DEFAULT 0"),
            ("prepay_tbl", "CREATE TABLE IF NOT EXISTS mortgage_prepay_scenarios(id INTEGER PRIMARY KEY AUTOINCREMENT, "
             "name TEXT NOT NULL, description TEXT DEFAULT '', annual_lump REAL DEFAULT 0, lump_month INTEGER DEFAULT 1, "
             "lump_start_year INTEGER DEFAULT 1, lump_num_years INTEGER DEFAULT 0, pay_increase_type TEXT DEFAULT 'None', "
             "pay_increase_val REAL DEFAULT 0, onetime_period INTEGER DEFAULT 0, onetime_amount REAL DEFAULT 0, "
             f"created_at TEXT DEFAULT (datetime('now')), updated_at TEXT DEFAULT (datetime('now')))"),
        ]
        for _nm, sql in migs:
            try: c.execute(sql)
            except Exception: pass
        ...
        migs = [
            ...
            "IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id=OBJECT_ID('mortgage_scenarios') AND name='linked_pp_db_id') "
            "ALTER TABLE mortgage_scenarios ADD linked_pp_db_id INT DEFAULT 0",
            "IF NOT EXISTS(SELECT * FROM sysobjects WHERE name='mortgage_prepay_scenarios' AND xtype='U') "
            "CREATE TABLE mortgage_prepay_scenarios(id INT IDENTITY PRIMARY KEY, name NVARCHAR(200) NOT NULL, "
            "description NVARCHAR(2000) DEFAULT '', annual_lump DECIMAL(15,2) DEFAULT 0, "
            "lump_month INT DEFAULT 1, lump_start_year INT DEFAULT 1, lump_num_years INT DEFAULT 0, "
...
def db_load_setup(conn):
    ...
    try:
        c = conn.cursor()
        sql = _top1(conn, "id, purchase_price, down_pct, mortgage_type, pay_frequency, "
                          "annual_rate, amort_years, term_years, start_date, include_cmhc",
                    "mortgage_setup", "id DESC")
        c.execute(sql)
        row = c.fetchone()
...
def db_save_setup(conn, data):
    ...
    try:
        ...
        print(f"db_save_setup error: {e}"); return False
...
def db_save_scenario(conn, sc_id_or_none, name, desc, renewals, pp_settings, user_pmt=0, linked_pp_db_id=0):
    ...
    try:
        ...
        lppid = int(linked_pp_db_id or 0)
        gd   = _getdate(conn)
        if sc_id_or_none:
            c.execute(f"UPDATE mortgage_scenarios SET name=?, description=?, "
                      f"annual_lump=?, lump_month=?, lump_start_year=?, lump_num_years=?, "
                      f"pay_increase_type=?, pay_increase_val=?, onetime_period=?, onetime_amount=?, "
                      f"user_pmt=?, linked_pp_db_id=?, updated_at={gd} WHERE id=?",
                      (name, desc,
            ...
            c.execute("INSERT INTO mortgage_scenarios "
                      ...
                      "pay_increase_type, pay_increase_val, onetime_period, onetime_amount, user_pmt, linked_pp_db_id) "
                      "VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                      (name, desc,
                       float(pp.get("annual_lump",0)), int(pp.get("lump_month",1)),
                       int(pp.get("lump_start_year",1)), int(pp.get("lump_num_years",0)),
        ...
        for i, rn in enumerate(renewals, 1):
            ...
            c.execute("INSERT INTO mortgage_scenario_renewals "
                      ...
                      (db_id, i, rn.get("mode","By Date"),
                       ...
                       float(rn.get("onetime_amount",0))))
        conn.commit()
        return db_id
    except Exception as e:
        print(f"db_save_scenario error: {e}"); return None
...
def db_load_scenarios(conn):
    ...
    try:
        ...
        c.execute("SELECT id, name, description, annual_lump, lump_month, lump_start_year, "
                  f"lump_num_years, pay_increase_type, pay_increase_val, onetime_period, onetime_amount, "
                  f"{ifn('user_pmt')}, {ifn('linked_pp_db_id')}, created_at "
                  "FROM mortgage_scenarios ORDER BY id")
        rows = c.fetchall()
        scenarios = []
        for row in rows:
            sid,name,desc,al,lm,lsy,lny,pit,piv,otp,ota,upmt,lppid,cat = \
                row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11],row[12],row[13]
            c2 = conn.cursor()
            c2.execute(f"SELECT seq_num, mode, effective_date, effective_period, new_rate, "
                       f"mortgage_type, term_years, actual_penalty, misc_fees, "
                       f"orig_posted_rate, curr_posted_rate, {ifn('onetime_amount')} "
                       f"FROM mortgage_scenario_renewals WHERE scenario_id=? ORDER BY seq_num", (sid,))
            renewals = []
            for r in c2.fetchall():
                sn,mode,edate,eper,nr,mt,ty,ap,mf,op,cp,ren_ota = r[0],r[1],r[2],r[3],r[4],r[5],r[6],r[7],r[8],r[9],r[10],r[11]
                renewals.append({
                    ...
                    "onetime_amount": float(ren_ota) if ren_ota else 0, "variable_subs": {},
                })
            scenarios.append({
                ...
                "linked_pp_db_id": int(lppid or 0),
                "created_at": str(cat)[:16],
            })
        return scenarios
    except Exception as e:
        print(f"db_load_scenarios error: {e}"); return []
...
def db_update_scenario(conn, db_id, name, desc, renewals, pp_settings, user_pmt=0, linked_pp_db_id=0):
    ...
    return db_save_scenario(conn, db_id, name, desc or "", renewals, pp_settings,
                            user_pmt, linked_pp_db_id) is not None
...
def db_save_prepay_scenario(conn, sc_id_or_none, name, desc, settings):
    ...
    try:
        ...
        gd = _getdate(conn)
        if sc_id_or_none:
            c.execute(f"UPDATE mortgage_prepay_scenarios SET name=?, description=?, "
                      f"annual_lump=?, lump_month=?, lump_start_year=?, lump_num_years=?, "
                      f"pay_increase_type=?, pay_increase_val=?, onetime_period=?, onetime_amount=?, updated_at={gd} WHERE id=?",
                      (name, desc,
        ...
        print(f"db_save_prepay_scenario error: {e}"); return None
...
def db_load_prepay_scenarios(conn):
    ...
    try:
        ...
        print(f"db_load_prepay_scenarios error: {e}"); return []
